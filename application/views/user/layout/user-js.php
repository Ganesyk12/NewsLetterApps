<script text="text/javascript">
   ...
   $(document).ready(function() {
      mainTable = $('#user-table').DataTable({
         ...
         "ajax": {
            "url": "<?= base_url('C_User/view_data_where'); ?>",
            "type": "POST",
            "data": function(data) {
         ...
         "columns": [{
            ...
            },
            {
               ...
               render: function(data, type, row, meta) {
                  ...
                  return data ? '<img src="' + baseUrl + data + '" alt="User Photo" width="80" height="80">' : '<img src="' + defaultImage + '" alt="User Photo" width="80" height="80">';
               }
            },
            {
               "data": "id_user",
               ...
               render: function(data, type, row, meta) {
                  mnu = '';
                  mnu = mnu + '<div class="btn btn-success btn-sm konfirmasiView" data-id="' + data + '" data-toggle="modal" data-target="#userModal" > <i class="fa fa-eye"></i></div>';
                  mnu = mnu + '<div class="btn btn-primary btn-sm konfirmasiEdit" data-id="' + data + '" data-toggle="modal" data-target="#userModal"> <i class="fa fa-edit"></i></div>';
   ...
   function Simpan_data($trigger) {
      ...
      var url = null;
      if ($trigger === 'Add') {
         url = "<?= base_url('C_User/regist') ?>";
      } else {
         url = "<?= base_url('C_User/ajax_update') ?>";
      }
      ...
      $.ajax({
         ...
         success: function(data) {
            ...
            mainTable.draw();
            var toastMessage = $trigger === 'Add' ? 'Data Saved successfully!' : 'Data Updated successfully!';
   ...
   $('#user-table').on('click', 'tr', function() {
      $('#iddelete2').text($('#user-table').DataTable().row(this).id());
   });

   function view_modal(id_user, status) {
      if (status == "Add") {
         $('#exampleModalLabel').text('ADD DATA USER');
         $('#quickForm')[0].reset();
         ...
         $('#role_id').select().val('').trigger('change');
         ...
         $('#id_user').val(id_user);
         $.ajax({
            url: "<?= base_url('C_User/ajax_getbyhdrid') ?>",
            method: "get",
            dataType: "JSON",
            data: {
               id_user: id_user
            ...
            success: function(data) {
               ...
               $('#nama').val(data.nama);
               $('#role_id').select().val(data.role_id).trigger('change');
               if (data.foto === null) {
   ...
   function Delete_data() {
      var fdata = new FormData();
      fdata.append('id_user', $('#iddelete').text());
      vurl = "<?= base_url('C_User/ajax_delete') ?>";
      $.ajax({
         url: vurl,
         method: "post",
         processData: false,
         ...
         success: function(data) {
            ...
            $('#user-table').DataTable().row("#" + $('#iddelete2').text()).remove().draw();
            // Show success message
            Swal.fire({
               position: "center",
               icon: "success",
